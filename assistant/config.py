...
DEFAULTS: dict = {
    ...
    "memory": {
        ...
        "working_graph": {
            ...
            "brief_max_chars": 1200,  # hard cap on the briefing block
            "brief_threads": 5,      # hottest edges shown as "threads"
            # Grounded frame lines: inject the actual source CLAUSE behind each held phrase (that
            # verbatim content is the memory; without it the block is a word list).  0 = always
            # inject; >0 = only once a node's text has scrolled that many turns off-screen.
            "ground_after_turns": 0,
            "brief_ground_max": 6,   # at most this many clause-bearing lines per briefing
    ...
    "tools": {
        ...
        "own_tools": {
            ...
            # Each pass asks two questions: (1) is a tool missing? → queue a PLAIN-LANGUAGE
